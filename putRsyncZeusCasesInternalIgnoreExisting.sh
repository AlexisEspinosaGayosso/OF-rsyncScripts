...
logRun="$hereD/log_runPutup_${dateString}"
logJob="$hereD/log_jobPutup_${dateString}"
...
#-----------------------------------------
#Puting up the basic directories
cd $hereD
#The 0 Dir ...
bDir=0
...
echo "CaseDir=${caseDir}" | tee -a ${logJob}
echo "#--------------------------------------------------" | tee -a ${logJob}
echo "Puting up the ${bDir} Dir without polyMesh" | tee -a ${logJob}
echo "#--------------------------------------------------" | tee -a ${logJob}
for ((jTries=0; jTries<$MaxTries; jTries++))
do
   echo "bDir=${bDir} putup will be done, try ${jTries}" | tee -a ${logJob}
   rsync --ignore-existing --stats --progress -auvzrhs --exclude polyMesh/ espinosa@hpc-data.pawsey.org.au:$caseDir/$bDir . | tee -a ${logJob}
...
#-----------------------------------------
#Puting up the QUICK postProcessing Directories
cd $hereD
...
#-----------------------------------------
#Defining the QUICK times directories to putup
cd $hereD
if [ "$doDirectories" = "true" ]; then
   ...
   if [ $nTimeDirectories -ge $NTheLast ]; then
       jIni=$NTheLast
       echo "Quick Puting up until the last ${NTheLast} times" | tee -a ${logJob}
   else
       jIni=$nTimeDirectories
       echo "Quick Puting up until the existing ${nTimeDirectories} times" | tee -a ${logJob}
   fi


   #-----------------------------------------
   #Puting up QUICK the last N existing times times
   
   echo "Quick Puting up the last ${jIni} times" | tee -a ${logJob}
   for ((j=$jEnd; j<$jIni; j++))
   do
       bDir=${timeDirArr[$j]}
       echo "Quick Puting up ${bDir}" | tee -a ${logJob}
       basicPutupIgnoreExisting
   done
fi
...
#-----------------------------------------
#Puting up the REST of postProcessing Directories
   cd $hereD
   echo "Puting up the rest of the postprocessing directory" | tee -a ${logJob}
   bDir=./postProcessing
   basicPutupIgnoreExisting
...
#Defining the REST of the time directories
   if [ "$doDirectories" = "true" ]; then
      ...
      if [ $nTimeDirectories -ge $Nlast ]; then
          jIni=$Nlast
          echo "Puting up until the last ${Nlast} times" | tee -a ${logJob}
      else
          jIni=$nTimeDirectories
          echo "Puting up until the existing ${nTimeDirectories} times" | tee -a ${logJob}
      fi
      cd $hereD
...
#backing up the REST of the time directories
      cd $hereD
      echo "Puting up the last ${jIni}+1 times" | tee -a ${logJob}
      for ((j=$jEnd; j<$jIni; j++))
      do
          bDir=${timeDirArr[$j]}
          echo "Puting up ${bDir}" | tee -a ${logJob}
          basicPutupIgnoreExisting
      done
   fi
fi
...
#-----------------------------------------
#End
echo "Putup of case ${caseDir} Finished" | tee -a ${logJob}
